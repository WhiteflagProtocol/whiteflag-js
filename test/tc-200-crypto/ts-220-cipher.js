...
/* Functions to test */
import { encryptMsg, decryptMsg, createAesKey } from '@whiteflagprotocol/crypto';

/* Test data */
import testVector from './data/tv-220-cipher.json' with { type: 'json' };
...
testCase('Test case 220: Crypto cipher module', function() {
    testCase('AES 256 counter mode', function() {
        assertion(' 1. should pass NIST SP 800-38A F.5.5 CTR-AES256 encryption', async function() {
            ...
            const encrypted = await encryptMsg(plaintext, AES256CTR_PSK, key, iv);
            return strictEqual(u8aToHex(encrypted), cyphertext);
        });
        assertion(' 2. should pass NIST SP 800-38A F.5.6 CTR-AES256 decryption', async function() {
            ...
            const decrypted = await decryptMsg(ciphertext, AES256CTR_PSK, key, iv);
            return strictEqual(u8aToHex(decrypted), plaintext);
        });
    });
});
