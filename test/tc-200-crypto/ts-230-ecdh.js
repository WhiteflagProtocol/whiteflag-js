    ...
    testCase('Shared secret negotiation', function() {
        ...
        assertion(' 1. should pass RFC 6932 Test Vector A.2', async function() {
            ...
            const keypairA = createExtKeyPair(
                createExtKey(hexToU8a(testVector['1']['A'].key), 'private', ECDHALG, ['deriveBits', 'deriveKey']),
                createExtKey(hexToU8a(testVector['1']['A'].pubkey), 'public', ECDHALG, ['deriveBits', 'deriveKey']),
            )
            const keypairB = createExtKeyPair(
                createExtKey(hexToU8a(testVector['1']['B'].key), 'private', ECDHALG, ['deriveBits', 'deriveKey']),
                createExtKey(hexToU8a(testVector['1']['B'].pubkey), 'public', ECDHALG, ['deriveBits', 'deriveKey']),
            )
            /* Create public keys */
        ...
        assertion(' 2. should generate two random keypairs with two equal secrets', async function() {
            ...
            /* Derive secrets */
            const secret1 = await deriveEcdhSecret(keypair1, keypair2.publicKey);
            const secret2 = await deriveEcdhSecret(keypair2, keypair1.publicKey);
            /* Verify secrets */
