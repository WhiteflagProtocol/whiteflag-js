...
class WfProtocol {
     ...
     * @param rules the set of rules to activate upon initialization
     * @returns the Whiteflag protocol layer singular instance
     * @throws if called before Whiteflag state has been initialized
     */
    public static async init(rules: WfProtocolRule[] = wfDefaultRuleSet): Promise<WfProtocol> {
        ...
        this.#instance.addRuleSet(rules);
    ...
    #createHandler(rule: WfProtocolRule): WfProtocolHandler {
        return function(data: WfEventData) {
            wfEvent.emit(rule.emits, rule.chain.execute(data));
        }
    }
}
