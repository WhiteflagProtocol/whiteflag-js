...
import { isBase64, objHas, b64ToU8a, hexToU8a, u8aToB64 } from '@whiteflagprotocol/util';
...
async function decryptData(key: CryptoKey, edo: EncryptedData): Promise<ByteArray> {
    /* Check data */
    if (!objHas(edo, 'iv') || !isBase64(edo.iv)) throw new TypeError('Encrypted data object does not contain a base64 encoded initialization vector');
    if (!objHas(edo, 'encrypted') || !isBase64(edo.encrypted)) throw new TypeError('Encrypted data object does not contain base64 encoded encrypted data');
