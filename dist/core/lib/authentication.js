...
import { arrayEquals, objHasNot, b64uToU8a, strToU8a, u8aToB64u } from '@whiteflagprotocol/util';
...
async function validateAuthSignature(blockchain, account, signature, url) {
    let errors = [];
    if (objHasNot(signature?.payload, 'addr')) {
    ...
    if (objHasNot(signature?.payload, 'url')) {
    ...
    if (objHasNot(signature?.payload, 'orgname')) {
