...
/* Functions to test */
import { WfRuntimeError, WfProtocolError, WfErrorCode, handleError } from '@whiteflagprotocol/common';
...
testCase('Test case 010: Common error module', function() {
    testCase('Error handling', function() {
        ...
        assertion(' 2. should correctly handle a TypeError', function(done) {
            throws(throwTypeError, WfRuntimeError);
            return done();
        });
        assertion(' 3. should correctly handle a general Error', function(done) {
            throws(throwError, WfRuntimeError);
            return done();
        });
        assertion(' 4. should correctly handle a WfProtocolError', function(done) {
            throws(throwWfProtocolError, WfProtocolError);
            return done();
        });
 ...
 * @throws a WfProtocolError
 */
function throwWfProtocolError() {
    try {
        throw new WfProtocolError(MESSAGE_WfError1);
    } catch (err) {
