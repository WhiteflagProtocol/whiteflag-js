...
class WfMessage extends WfCoreMessage {
     ...
     */
    public static async fromObject(message: any): Promise<WfMessage> {
        const wfMessage = await super.fromObject(message) as WfMessage;
     ...
     */
    public static async fromHex(message: string, account?: WfAccount, ikm?: string, iv?: string): Promise<WfMessage> {
        const wfMessage = await super.fromHex(message, account, ikm, iv) as WfMessage;
        if (account) wfMessage.setMeta('originatorAddress', account.address);
     ...
     * @returns a new Whiteflag message object with the decoded message
     */
    public static async fromU8a(message: Uint8Array, account?: WfAccount, ikm?: Uint8Array, iv?: Uint8Array): Promise<WfMessage> {
        const wfMessage = await super.fromU8a(message, account, ikm, iv) as WfMessage;
        if (account) wfMessage.setMeta('originatorAddress', account.address);
    ...
    public toObject(): Object {
        const message = super.toObject() as any;
